...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, e =>e.Mgr, em => em.EmpNo, (e, em) => new { Employee = e.EName, Manager = em.EName }).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var empsDepts = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e.EName, d.DName, e.Sal}).ToList();

        var result = empsDepts.SelectMany(ed => grades.Where(s=> ed.Sal >= s.Losal && ed.Sal <= s.Hisal), (ed, s) => new {ed.EName, ed.DName, s.Grade}).ToList();
